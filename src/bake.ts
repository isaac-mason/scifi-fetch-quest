 ...
 *
 * A minimal scene - just the ship splat - so each capture integrates only the environment. The
 * .spz loads with `nonLod: true`, decoding the whole splat into a non-paged set, so
 * `splat.initialized` resolving guarantees full residency (no per-probe streaming).
 *
 * The grid box is fit to the collider's AABB (the hand-authored mesh frames the interior tightly),
 * inset a little, then filled with a dense lattice: every cell is an order-2 SH probe, packed into
 * a 3D-texture atlas, shipped as light-probes.json. Intensity + saturation are baked into the SH.
 *
 * scripts/bake-probes.mjs drives this page in real Chrome and writes public/light-probes.json via
 * window.__saveProbes; it also dumps a raw six-face strip (window.__saveDebugPng). Opened manually
...
// Capture the six per-face renders at `pos` (raw) into a horizontal strip data URL - exactly what
// the SH sees, to confirm all six faces are populated + distinct.
...
async function main(): Promise<void> {
    // Square canvas at the capture resolution: each cube face renders to the canvas and reads back,
    // so its size is the face size. 128 is plenty for order-2 SH.
    const FACE_SIZE = 128;
    ...
    // nonLod: true decodes the whole .spz into a non-paged set, so `splat.initialized` resolving
    // means every splat is resident. Bake from the .spz, not the runtime .rad, for full residency.
    ...
    // Warm-up: render real frames (yielding each time) so Spark builds its splat instances + first
    // sort before capturing. The per-face settle in captureCubeFaces handles the rest.
    for (let i = 0; i < 30; i++) {
        renderer.render(scene, camera);
        await new Promise((r) => requestAnimationFrame(() => r(undefined)));
    }

    // Fit the probe box to the collider's AABB, then inset so edge cells sit just inside the hull.
    // The collider's bounds already frame the interior - no splat-occupancy fitting needed.
    ...
    // Dense regular grid: every cell is baked (a 3D-texture volume can't ship holes; a void cell
    // just captures dark). Resolution + cell order match the atlas packer.
    ...
    // Guard: a mis-fit box blows up a dense grid fast (nx*ny*nz). Bail loudly - raise PROBE_SPACING
    // to coarsen the grid, or shrink the collider/inset.
    ...
    // Diagnostic: dump the raw six per-face renders of a central probe to confirm all six faces
    ...
    // Six per-face renders per probe. Bake PROBE_SATURATION into the SH so the atlas carries the
    // chroma boost. Coefficients come back in `positions` order.
    ...
    // multiplier, so it must live in the atlas.
    ...
    // Pack into the 3D-texture atlas layout the volume shader reads (see light-probes.ts).
